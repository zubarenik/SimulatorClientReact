...
import { NavLink, useLocation, matchPath, useParams, withRouter } from 'react-router-dom'
...
const VerticalNavMenuLink = ({
  ...
  const searchParents = (nav, currentURL) => {
    const parents = search(nav, currentURL, routerProps) // Search for parent object
  ...
  const resetActiveGroup = navLink => {
    const parents = search(nav, navLink, match)
  ...
  useEffect(() => {
    ...
    if (currentActiveItem !== null) {
      ...
      const arr = searchParents(nav, currentURL)
      setGroupActive([...arr])
    }
  }, [location])
    ...
    >
      <LinkTag
        ...
        {...(item.externalLink === true
          ...
          : {
              to: item.navLink.replace(":sim_id", sim_id) || '/',
              isActive: (match, location) => {
                if (!match) {
                  return false
                }

                if (match.url && match.url !== '' && match.url === item.navLink.replace(":sim_id", sim_id)) {
...
export default withRouter(VerticalNavMenuLink)
