...
export default class Actions {
  ...
  static async find(params){
    let response = await api.get("theory_chapters.json", params)
    if (response.ok) {
      store.dispatch({
        type: "THEORY_INDEX",
  ...
  static async create(data){
    ...
    let response = await api.post("theory_chapters.json", formData, undefined)
    if (response.ok) {
  ...
  static async edit(data){
    if(data.newpicture)
    {
      ...
      let response = await api.patch(`theory_chapters/${data.id}.json`, formData)
      if (response.ok) {
        Actions.find({simulator: data.simulator})
      }
      return response
